...
class RolesController {
  ...
  public async getOne(req: Request, res: Response) {
    const { id } = req.params;
    let getOne = await pool.query("SELECT * FROM roles WHERE id_roles = ?", [
      id,
    ]);
  ...
  public async create(req: Request, res: Response) {
    ...
    if (req.body.name_role == "") {
      ...
      let exits = await pool.query("SELECT * FROM roles WHERE name_role = ?", [
        req.body.name_role,
      ]);
      if (Object.entries(exits).length === 0) {
        ...
        res
          .status(404)
          .json({ status: false, message: "El nombre del rol ya existe." });
      }
    }
  }
  ...
  public async update(req: Request, res: Response) {
    ...
    let sql = await pool.query(
      "SELECT * FROM roles WHERE name_role = ? AND id_roles != ?",
      [req.body.name_role, id]
    );
    if (Object.entries(sql).length === 0) {
      let update = await pool.query("UPDATE roles SET ? WHERE id_roles = ?", [
        req.body,
        id,
      ]);
      if (update) {
      ...
      res
        ...
        .json({
          status: false,
          message: "El nombre del rol ya existe en otro id.",
        });
    }
  }
  ...
  public async delete(req: Request, res: Response) {
    ...
    let exists = await pool.query(
      "SELECT * FROM roles_users WHERE pk_fk_id_roles = ?",
      [id]
    );
    if (Object.entries(exists).length === 0) {
      let role_status = 0;
      let update = await pool.query(
        "UPDATE roles SET role_status = ? WHERE id_roles = ?",
        [role_status, id]
      );
      if (update) {
        res
      ...
      res
        ...
        .json({
          status: false,
          message:
            "El rol esta asociado a uno o varios empleados no se puede eliminar.",
        });
    }
  }
}
