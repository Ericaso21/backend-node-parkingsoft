...
class UserController {
    ...
    public async getOne (req: Request, res: Response){
        const { id } = req.params;
        let user = await pool.query("SELECT u.document_number, u.pk_fk_id_document_type, u.fk_id_gender, tp.document_type_acronym, g.name_gender, u.first_name,u.second_name, u.surname,u.second_surname, u.email, r.id_roles, r.name_role, u.telephone, ru.roles_users_status, DATE_FORMAT(u.created_att, '%d-%m-%Y') as fechaCreacion FROM users u INNER JOIN roles_users ru ON u.document_number = ru.pk_fk_document_number INNER JOIN roles r ON ru.pk_fk_id_roles = r.id_roles INNER JOIN document_types tp ON u.pk_fk_id_document_type  = tp.id_document_type INNER JOIN genders g ON u.fk_id_gender  = g.id_gender WHERE u.document_number = ?",[id]);
    ...
    public async delete (req: Request, res: Response){
        ...
        if (Object.entries(update_vehicle).length === 0) {
            let update_ticket = await pool.query('SELECT * FROM tickets WHERE pk_fk_document_number = ?',[id]);
            if (Object.entries(update_ticket).length === 0) {
                let update_bill = await pool.query('SELECT * FROM bills WHERE pk_fk_document_numer = ?',[id]);
                if (Object.entries(update_bill).length === 0) {
                    let roles_users_status = 0;
                    let update_status = await pool.query('UPDATE roles_users SET roles_users_status = ? WHERE pk_fk_document_number = ?',[roles_users_status,id]);
                    if (update_status) {
                        res.status(200).json({status: true, message: 'Usuario eliminado con exito.'});
                    }
                } else {
                    res.status(404).json({status: false, message: 'El usuario no se puede eliminar por que esta asociado algunas facturas.'});
                }
            } else {
                res.status(404).json({status: false, message: 'El usuario no se puede eliminar por que esta asociado algunos tickets'});
            }
        } else {
