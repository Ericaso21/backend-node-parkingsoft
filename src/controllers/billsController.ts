...
class BillController {
  ...
  public async create(req: Request, res: Response) {
    ...
    if (req.body.fk_id_ticket == 0) {
      ...
      let rates = await pool.query(
        "SELECT r.id_rate, r.fk_id_vehicle_type, r.minute_rate, IF(CHAR_LENGTH(FORMAT(r.hourly_rate,3)) = 7, r.hourly_rate, FORMAT(r.hourly_rate,3)) AS hourly_rate, IF(CHAR_LENGTH(FORMAT(r.day_rate,3)) = 7,r.day_rate,FORMAT(r.day_rate,3)) AS day_rate FROM rates r WHERE r.rate_status !=0"
      );
      let time = await pool.query(
        "SELECT IF(t.entry_time<t.departure_time, TIMESTAMPDIFF(SECOND,t.entry_time,t.departure_time), TIMESTAMPDIFF(SECOND,t.departure_time,t.entry_time)) AS total_time, v.fk_id_vehicle_type FROM tickets t INNER JOIN tickets_block tb ON t.id_ticket = tb.pk_fk_id_ticket INNER JOIN vehicles v ON tb.fk_id_vehicle = v.vehicle_plate WHERE t.id_ticket = ?",
        [req.body.id_ticket]
      );
      ...
      if (insert) {
        ...
        res
          .status(500)
          .json({ status: false, message: "No se pudo insertar." });
      }
    }
  }
  ...
  public async deleteBill(req: Request, res: Response) {
    ...
    let update_bill = await pool.query(
      "UPDATE bills SET bill_status = 0, updated_att = ? WHERE id_bill = ?",
      [updated_att, id]
    );
    if (update_bill) {
      res
