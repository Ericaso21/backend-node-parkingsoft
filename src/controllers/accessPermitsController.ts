...
class AccessPermitsController {
  ...
  public async getOne(req: Request, res: Response) {
    const { id } = req.params;
    let permit = await pool.query(
      "SELECT * FROM access_permits WHERE id_access_permits = ?",
      [id]
    );
  ...
  public async create(req: Request, res: Response) {
    ...
    if (req.body.fk_id_roles == 0 || req.body.fk_id_modules == 0) {
      ...
      let exist = await pool.query(
        "SELECT * FROM access_permits WHERE fk_id_roles = ? AND fk_id_modules = ?",
        [req.body.fk_id_roles, req.body.fk_id_modules]
      );
      if (Object.entries(exist).length === 0) {
        // crete accesPermit database
        let insert = await pool.query("INSERT INTO access_permits SET ?", [
          req.body,
        ]);
        if (insert) {
          res
        ...
        res
          ...
          .json({
            status: false,
            message: "El permiso ya esta asociado a un rol y modulo.",
          });
      }
    }
  }
  ...
  public async update(req: Request, res: Response) {
    ...
    const { id } = req.params;
    let exist = await pool.query(
      "SELECT * FROM access_permits WHERE (fk_id_roles = ? AND fk_id_modules != ?) AND (id_access_permits = ?)",
      [req.body.fk_id_roles, req.body.fk_id_modules, id]
    );
    if (Object.entries(exist).length === 0) {
      let update = await pool.query(
        "UPDATE access_permits SET ? WHERE id_access_permits = ?",
        [req.body, id]
      );
      if (update) {
        res
      ...
      res
        ...
        .json({
          status: false,
          message: "El permiso ya esta asociado a otro ID.",
        });
    }
  }
  //delete permits role
  public async delete(req: Request, res: Response) {
    const { id } = req.params;
    await pool.query("DELETE FROM access_permits WHERE id_access_permits = ?", [
      id,
    ]);
    res
      .status(200)
