...
class VehicleController {
  ...
  public async list(req: Request, res: Response) {
    let vehicles = await pool.query(
      "SELECT v.vehicle_plate, tv.vehicle_name, v.fk_document_number, v.model_number, v.vehicle_status FROM vehicles v INNER JOIN vehicle_types tv ON v.fk_id_vehicle_type = tv.id_vehicle_type WHERE v.vehicle_status !=0"
    );
  ...
  public async getOne(req: Request, res: Response) {
    ...
    let vehicle = await pool.query(
      "SELECT * FROM vehicles WHERE vehicle_plate = ?",
      [id]
    );
  ...
  public async create(req: Request, res: Response) {
    ...
    if (
      ...
      req.body.fk_document_number == "" ||
      req.body.fk_id_document_type == 0 ||
      req.body.fk_id_vehicle_type == 0 ||
      req.body.vehicle_status == 0
    ) {
      res
        .status(404)
      ...
      if (Object.entries(exist).length === 0) {
        let fk_id_document_type = await pool.query(
          "SELECT pk_fk_id_document_type FROM users WHERE document_number = ?",
          [req.body.fk_document_number]
        );
        if (Object.entries(fk_id_document_type).length === 0) {
          res
            .status(404)
            .json({ status: false, message: "No existe el usuario" });
        } else {
          let vehicle = {
            vehicle_plate: req.body.vehicle_plate,
            fk_document_number: req.body.fk_document_number,
            fk_id_document_type:
              fk_id_document_type[0]["pk_fk_id_document_type"],
          ...
          let insert = await pool.query("INSERT INTO vehicles SET ?", [
            vehicle,
          ]);
          if (insert) {
            res
            ...
            res
              .status(500)
              .json({ status: false, message: "No se ha podido guardar." });
          }
        }
      } else {
  ...
  public async update(req: Request, res: Response) {
    ...
    if (Object.entries(exist).length === 0) {
      ...
      res
        .status(404)
        .json({ status: false, message: "El vehiculo ya existe." });
    }
  }
  //delete vehicle
  public async delete(req: Request, res: Response) {
    ...
    let update_status = await pool.query(
      "UPDATE vehicles SET vehicle_status = ? WHERE vehicle_plate = ?",
      [vehicle_status, id]
    );
    if (update_status) {
      res
