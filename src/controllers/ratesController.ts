...
class RatesController {
    ...
    public async update(req: Request, res: Response){
        delete req.body.token;
        const { id } = req.params;
        let exist = await pool.query('SELECT * FROM rates WHERE fk_id_vehicle_type = ? AND id_rate != ?',[req.body.fk_id_vehicle_type,id]);
        if (Object.entries(exist).length === 0) {
            let update = await pool.query('UPDATE rates SET ? WHERE id_rate = ?',[req.body,id]);
            if (update) {
                res.status(200).json({status: true, message: 'Datos actualizados correctamente.'});
            } else {
                res.status(500).json({status: false, message: 'No se pudo actualizar'});
            }
        } else {
            res.status(404).json({status: false, message: 'El tipo de vehiculo ya existe para otra tarifa.'});
        }
    }
    //delete rate
    public async delete (req: Request, res: Response){
        const { id } = req.params;
        let rate_status	= 0;
        let update = await pool.query('UPDATE rates SET rate_status	= ? WHERE id_rate = ?',[rate_status,id]);
        if (update) {
            res.status(200).json({status: true, message: 'Se ha eliminado correctamente la tarifa.'});
        } else {
            res.status(500).json({status: false, message: 'No se ha podido eliminar la tarifa.'});
        }
    }
