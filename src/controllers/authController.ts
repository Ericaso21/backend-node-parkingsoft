...
class AuthController {
    ...
    public async create(req: Request, res: Response) {
        ...
        if (req.body.document_number == '' || req.body.pk_fk_id_document_type === 0, req.body.fk_id_gender == 0 || req.body.email == '' || req.body.password_user == '') {
            ...
            if (Object.entries(exist).length === 0) {
                ...
                if (insert) {
                    ...
                    if (role_user) {
                        ...
                        res.status(200).json({ status: true, message: 'Registro exitoso.' });
                    }
                } else {
    ...
    public async authentication(req: Request, res: Response) {
        ...
        let user_auth = await pool.query('SELECT u.first_name, u.second_name, u.surname, u.second_surname, u.email FROM users u INNER JOIN roles_users rl ON u.document_number = rl.pk_fk_document_number WHERE u.email = ? AND rl.roles_users_status !=2', [req.body.email]);
